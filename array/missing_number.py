...
def missing_first_positive(nums:list)->int:
    ...
    n = len(nums)
    for i in range(n):
        ## why use a while loop not simly a if statement?
        ## cuz one swap might not be enough
...
def disappeared_numbers(nums:list)->list:
    ...
    After finish traversing for the first time, return whoseever number does not correspond to index
    :param nums: list[int], where each element [1:len(n)]
    :return: list[int], the numbers from [1:len(n)] not in nums
    """
    # moved element to its index ( ex. move 1 to nums[0] )
    for i in range(len(nums)):
        if nums[i]!=i+1:
            swap_helper(nums,nums[i])
    return [i+1 for i, num in enumerate(nums) if num!=i+1]

def swap_helper(nums:list,value:int):
    """
    a helper function of disappeared_numbers
    :param nums:
    :param value:
    :return:
    """
